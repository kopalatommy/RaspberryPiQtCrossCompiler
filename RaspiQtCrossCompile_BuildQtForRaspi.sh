...
cp toolchain_aarch.cmake ${BUILD_LOC}/qt6/toolchain.cmake

cd ${BUILD_LOC}/qt6/pi-build
# cmake ../src/qt-everywhere-src-$QtVersion/ -GNinja -DCMAKE_BUILD_TYPE=Release -DINPUT_opengl=es2 -DQT_BUILD_EXAMPLES=OFF -DQT_BUILD_TESTS=OFF -DQT_HOST_PATH=${BUILD_LOC}/qt6/host -DCMAKE_STAGING_PREFIX=${BUILD_LOC}/qt6/pi -DCMAKE_INSTALL_PREFIX=/usr/local/qt6 -DCMAKE_TOOLCHAIN_FILE=${BUILD_LOC}/qt6/toolchain.cmake -DQT_QMAKE_TARGET_MKSPEC=devices/linux-rasp-pi4-aarch64 -DQT_FEATURE_xcb=ON -DFEATURE_xcb_xlib=ON -DQT_FEATURE_xlib=ON
# cmake ../src/qt-everywhere-src-$QtVersion/ -GNinja -DCMAKE_BUILD_TYPE=Release -DINPUT_opengl=es2 -DQT_BUILD_EXAMPLES=OFF -DQT_BUILD_TESTS=OFF -DQT_HOST_PATH=${BUILD_LOC}/qt6/host -DCMAKE_STAGING_PREFIX=${BUILD_LOC}/qt6/pi -DCMAKE_INSTALL_PREFIX=/usr/local/qt6 -DCMAKE_TOOLCHAIN_FILE=${BUILD_LOC}/qt6/toolchain.cmake -DQT_QMAKE_TARGET_MKSPEC=devices/linux-rasp-pi4-aarch64 -DQT_FEATURE_xcb=ON -DFEATURE_xcb_xlib=ON -DQT_FEATURE_xlib=ON
BUILD_LOC=${BUILD_LOC}  ../src/qt-everywhere-src-$QtVersion/configure -release -opengl es2 -nomake examples -nomake tests -qt-host-path ${BUILD_LOC}/qt6/host -extprefix ${BUILD_LOC}/qt6/pi -prefix /usr/local/qt6 -device linux-rasp-pi4-aarch64 -device-option CROSS_COMPILE=/opt/cross-pi-gcc/bin/aarch64-linux-gnu- -- -DCMAKE_TOOLCHAIN_FILE=${BUILD_LOC}/qt6/toolchain.cmake -DQT_FEATURE_xcb=ON -DFEATURE_xcb_xlib=ON -DQT_FEATURE_xlib=ON -DBUILD_LOC=${BUILD_LOC} -DCMAKE_CXX_COMPILER_LAUNCHER=ccache
